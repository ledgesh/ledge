...
- **R6a.** A right-click has a canonical home too, and for a long time the
  ...
  listed (§11).
- **R6b.** A list's blank space opens the list's own add menu: right-clicking
  below the last workspace offers New Workspace and Attach Folder as
  Workspace…, and right-clicking below the last note offers New Note and New
  Folder…. It is the same menu the split button's chevron drops, from the same
  state, so the two surfaces cannot drift. The rule is the one R6a states for
  the editor, applied to a list: a right-click is answered by whatever the
  pointer landed on, and what it landed on here is the list rather than a row.
  The predicate is `onBlankSpace` (`lib/useRowMenu.ts`), which asks whether a
  row sits under the click: rows publish `data-target-kind` (§7), and a
  right-click on one opened that row's menu before the list's own handler saw
  it. Blank space takes no long press, unlike a row. Both verbs are already
  under a finger on the split button at the foot of the list, and a press held
  on a list is the gesture the list's own scroll wants (§1a).
...
| ⌘V of a picture | Paste in the text callout (Android's selection toolbar), which embeds a copied picture the way ⌘V does (ios.md §11); Insert Image…, on the bar and in the palette, for one nobody copied |
...
- **The editor's own menu (§11) is the exception, and takes no long press.**
  ...
  Chromium on Android raises a `contextmenu` for that press, which WebKit on
  iOS does not. The editor's menu ignores a touch or pen `contextmenu`, and the
  window-wide suppression lets one on text through (`pressIsSelection` in
  `lib/useRowMenu.ts`), so Android's own selection and its Cut, Copy and Paste
  toolbar come up, as iOS's callout does.
...
| Lock This Note… / Remove Lock… | — (palette / note row menu) | two faces, exactly one visible per the note's live locked flag (the template-marker move). Target-scoped like Delete: the sidebar row's menu acts on the row's note, the palette on the focused tab. Lock sweeps the note's images (sealed in place; shared ones surfaced as a notice) and runs vault setup/unlock first when needed, landing on its own confirm as the follow-up. That confirm is not §4-destructive either and is not asking whether to lock: it is where the history caveat is said, that encrypting now cannot retract the plaintext a sync service, a backup, or a git commit already holds (locking.md §1, §6a). Remove Lock sits behind one confirm — not §4-destructive (nothing is destroyed), but the consequence is silent exposure: sync and agent scans see the body again. Both live in locking.md §7 |
...
| Insert Image…         | — (palette; the accessory bar on a phone) | asks the DEVICE for a picture and embeds it: the file dialog on a Mac; on a phone, a menu of Photo Library, Take Photo and Choose File, then that screen (ios.md §11). Android's menu says Photos for the first, and its Take Photo asks for the camera permission first, because the manifest declares it for the pairing scanner and the system camera refuses an app that has not been granted a permission it declares (android/.../ImagePicker.kt). No chord, because ⌘V is already the desktop's way in for the pasteboard and this is the other source; on a phone the pasteboard's way in is the callout's Paste, and this is the way to a picture nobody copied. A cancel inserts nothing and says nothing, whether at the menu or in the picker: cancelling is the common outcome, not a failure. The caret ends up below the image's line, so it renders straight away — the same insert ⌘V does |
...
| Favorite / Unfavorite | `f` (also note row menu / palette) | puts the note in the browser's Favorites section, or takes it out. Target-scoped like Delete: the row's menu acts on the row's note, the palette on the focused tab's. ONE command with a LIVE TITLE rather than the two-faces pair the template and lock markers use, for Expand / Collapse's reason: it holds a bare key, and two commands may not claim one bare key on one row kind (the dispatcher resolves a bare key without consulting `when`). The marker is a `favorite: true` frontmatter line Bun writes into the note (`bun/notes.ts` favoriteNote), not a list in settings or in the layout: which notes those are is a fact about the notes, the `template:` marker's argument (architecture.md §6a), so it survives a retitle, travels through a move, syncs with the file, and can be typed by hand. The section reads the flag off the per-folder note lists the browser already holds, so marking one takes no query and no restart. The command SAVES the note's open buffers before the line lands, because Bun is editing the file underneath them and an unsaved buffer would carry the pre-marker text into the divergence guard, taking the marked file to the trash (`notes/actions.ts` favoriteNoteNow); the marked text then reaches the editor as any external edit does. A LOCKED note takes the marker with its vault shut, where Move to Folder… has to refuse one: the line goes in the plaintext head beside the note's tags and no body is read (locking.md §2). The manual's pages have no star and no menu item, and Bun refuses the write regardless. In the Note menu it sits above the marker pairs, acting on the focused tab's note like everything else there |
| Move to Folder…       | `m` (also note row menu / palette) | files a note in another folder of its OWN workspace (another workspace is the next row). Target-scoped like Delete — the row's menu acts on the row's note, the palette on the focused tab's. The destination is a chooser (`components/FolderPicker.tsx`) rather than an argument, and the chooser is also where a folder is CREATED: type a name no folder answers to and the last row offers to make it, which is the overlay's "offer the next mode instead of reporting the emptiness" (§1a) applied to a list of folders. Nothing exists on disk until a row is picked. The note keeps the name its heading gave it, suffixed only if the destination already holds it; its docId, editor, undo history and shell all live through the move (architecture.md §4), and Bun rewrites its image references for the new folder. A LOCKED note whose vault is shut is refused — those references are inside the encrypted body (`locking.md` §5). A drag of the row onto a folder is the same operation (R4: a pointer gesture, not a second command), and the Notes header is the drop target for the top level, which is the one destination with no row |
| New Note in Folder    | — (folder row menu)       | an Untitled note inside that folder, opened with the caret on its title. The file is written NOW, unlike ⌘N's tab-that-may-never-be-typed-in: the browser shows the folders its notes are in, so a deferred create would file the note into a row that is not there yet |
| New Folder…           | — (palette; the New Note button's dropdown; the browser's blank space; folder row menu) | the same chooser, and it makes the folder AND the first note in it, for the reason above — a folder with nothing in it is a folder the browser cannot draw. On a folder row it seeds the field with that folder, so nesting is a name and an Enter. The New Note split button carries it because a workspace with no folders yet has no folder row to open a menu on, which would leave the palette as the only door to the feature. That dropdown names New Note above it, as the strip's names New Workspace: a dropdown hanging off a split button says what the wide half does before it says what only the menu does, and R6b hands the same menu to a right-click that has no wide half beside it |
| Search in Folder      | `/` on a focused folder row (also its menu) | opens the overlay narrowed to that folder and the folders inside it, in text mode: `folder` on the noteSearch and tagList RPCs, which is the scope `ledge search -f` and the MCP tools' `folder` have had since agents got folders (architecture.md §3). The scope belongs to the OVERLAY, not to one of its modes — the chips carry it across the way they carry the query, since what you were looking IN is as much of the question as what you were looking for. It shows as a removable pill in the field, and that visibility is a correctness rule rather than a courtesy: a scoped search that found nothing is indistinguishable from an empty workspace unless the narrowing is on screen. Removed by clicking the pill or by Backspace at an empty field, which is where the mode sigils' own Backspace already put that gesture. Hidden while the Commands chip is lit, because commands are not in a folder and a scope over a list it does not narrow would be a claim about those rows that is not true; it is not forgotten, and crossing back brings the pill and its rows back together. The one folder verb ABOVE the read-only gate (the manual's own pages are worth searching a section of), and the one that earns a bare key: `/` is what every list-shaped program spells search with, and the folder row had only Enter spent on it |
| Move to Workspace…    | — (note row menu / palette)       | moves a note to the TOP LEVEL of another workspace on the same server. Target-scoped like Move to Folder…. The destination is a chooser of the strip's rows minus this one (`components/WorkspacePicker.tsx`), a list and not a field, since every row is a destination that exists. Disabled while the strip has one row: there is nowhere to go. No bare key, because `m` is spent and a note leaves its workspace rarely. What travels: the file under the name its heading gave it (suffixed only if the destination's top level holds it), its open tabs with their docId, editor, undo history and shells (`workspace/store.tsx` noteMovedWorkspace lifts them into the destination's focused pane, because a tab left behind would name a file in another workspace's folder, which the restore path prunes), and its images, which Bun COPIES into the destination's asset pool before rewriting the references (architecture.md §3). What does not: the wikilinks that named it from here, since a title resolves within one workspace, and the strip's sentence says how many notes those are ("Moved “Plan” to “Work”. 2 notes here link to it."). A LOCKED note is refused with the vault shut, for Move to Folder…'s reason, and travels sealed with it open. Reversible, so no confirmation and the Undo strip (§4): Undo is the same move back, run from the strip of the workspace the note left, which is where the strip is. A drag of the row onto a workspace row in the strip is the same operation (R4), through the same `moveAcross` in NoteBrowser.tsx; the selected row takes no drop, since the browser lists that workspace and the note is already in it. Not offered for folders: a folder move would be N of these, and nobody has asked |
| Rename Folder…        | `r` on a focused folder row (also its menu) | the row becomes a text field (`components/RenameField.tsx`, the workspace strip's own rename one register down) and Bun renames the directory. ONE rename(2), not a move of every note: the name changes and the PARENT does not, so nothing under the folder changes depth, every note's image references stay right, and no note's bytes are read or written. Three things fall out of that. It is atomic however many notes are under it. A folder full of LOCKED notes renames with the vault shut, where Move to Folder… has to refuse a single one (nothing inside a body is about to become wrong). And the answer is the notes' own metadata with new paths, not a re-read. The field therefore takes a NAME and refuses a path (`shared/folders.ts` folderLeafProblem): moving a folder is a different question — which parent — and it would have to rebase every note it moved. Refused too: a name a sibling already answers to (rename(2) swallows an empty directory in silence and fails with a raw ENOTEMPTY on a full one, and merging is not a rename), and a name `.ledgeignore` covers, which would take every note in the folder off the list at once (`ensureFolder`'s reason). A case-only change is a rename and not a collision, since APFS answers for both spellings. `r` is free here because a bare key is paired with a row KIND, so this is the workspace row's own mnemonic. No double-click, unlike that row: a click on a folder row toggles it, so a double-click is already two acts of the row's Enter verb (R6). Open tabs follow every note that moved and the open folders stay open under their new names (`notes/actions.ts` renameFolderTo), and the browser puts focus back on the row — a folder row's id is its path, so a rename replaces the row rather than relabelling it, and R5 would otherwise drop you out of the list |
