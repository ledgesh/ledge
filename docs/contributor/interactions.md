...
| New Folder…           | — (palette; the New Note button's dropdown; folder row menu) | the same chooser, and it makes the folder AND the first note in it, for the reason above — a folder with nothing in it is a folder the browser cannot draw. On a folder row it seeds the field with that folder, so nesting is a name and an Enter. The New Note split button carries it because a workspace with no folders yet has no folder row to open a menu on, which would leave the palette as the only door to the feature |
