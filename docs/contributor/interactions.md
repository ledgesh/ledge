...
| Folder    | Expand / Collapse | —                          | `/` Search in Folder; menu: Search in Folder, New Note in Folder, New Folder… (nested). No destructive verb: a folder is not a thing on disk the browser owns — it is where notes are, and it stops being a row when the last note leaves it |
