# Ledge note locking — design

**Status: design, accepted 2026-07-18, not yet implemented.** This document
becomes normative when the feature lands; until then the sibling standards
(`architecture.md`, `interactions.md`, `testing.md`) describe the app as it
is, and this one describes what locking must be when it exists. §7 here is
written to merge into `interactions.md` at that point; the seam rules in §4
merge into `architecture.md` §§3, 6a. Where this document and the shipped
code eventually disagree, the standing rule applies: one of them is wrong —
fix deliberately.
...
- **Encrypted at birth.** A paste into a locked note writes ciphertext from
  the first byte, named `<uniqueName>.png.enc` — the `.enc` suffix is
  honest: external tools fail loudly instead of parsing gibberish as a PNG.
  `assetPathOf`'s allowlist admits the suffix (an `.enc` whose inner
  extension is on the image allowlist). Because encryption happens at
  write, the deliberate never-unlink orphaning of assets (architecture §3)
  stays a storage quirk and never becomes a privacy leak.
...
- **Locking an existing note sweeps its referenced assets**: each in-root
  image reference is encrypted in place (temp-plus-rename; the markdown
  reference is rewritten to the `.enc` name in the same save). An asset
  also referenced by an *unlocked* note is surfaced as a conflict for the
  user to resolve, never silently decided — rare by construction, since
  paste allocates unique names and sharing only happens by hand. Removing
  the lock reverses the sweep for assets no other locked note references.
...
- **The title** — as the filename (was always visible), the H1, and
  ...
  lock dialog's one-line help says exactly that. Opaque filenames are a
  possible v2, not a v1 promise.
...
- **Row treatment**: locked notes wear the Lock glyph in the sidebar icon
  ...
  layout = template, calendar = daily seed, **lock = locked**. Tabs stay
  plain except the locked-placeholder face, which is its own evidence. No
  bare-key row verb: locking is a rare act, and bare keys are for the verbs
  a row does daily.
...
**The invariant: no agent surface returns locked plaintext, ever — vault
state is irrelevant to agents.** The lock is *for* them (§1), and it holds
...
- **e2e (headless WebKit)**: interposed unlock flow (open locked note →
  ...
  render; palette two-faces pair shows the right face.
