...
Hand-written tooltip strings are forbidden: the advertised-but-unbound ⌘W bug
happened because a tooltip and a binding were maintained separately. Buttons
...
- The window has exactly **one** keydown listener (`CommandProvider`), bubble
  phase. It skips events with `defaultPrevented` set.
