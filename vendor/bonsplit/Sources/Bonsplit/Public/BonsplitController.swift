...
public final class BonsplitController {
    ...
    /// Rename a tab in place and notify the delegate.
    ///
    /// Unlike `updateTab`, this is the user-initiated path: it reports the new
    /// title back through `didRenameTab` so the host can persist it.
    /// - Parameters:
    ///   - tabId: The tab to rename
    ///   - title: The new title. Blank titles are ignored.
    @discardableResult
    public func renameTab(_ tabId: TabID, to title: String) -> Bool {
        let trimmed = title.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty, let (pane, tabIndex) = findTabInternal(tabId) else { return false }

        pane.tabs[tabIndex].title = trimmed
        let renamed = Tab(from: pane.tabs[tabIndex])
        delegate?.splitTabBar(self, didRenameTab: renamed, inPane: PaneID(id: pane.id.id))
        return true
    }
