...
// phone's forced command. On a Mac it also installs the darwin tarball into a
// scratch HOME. Run `bun run build:server -- --targets=<these>` first.
...
const { BUN_VERSION, releaseName, renderInstallScript, SUMS_FILE } = await import("../src/bun/serverRelease");
...
// The "newer" release the update step installs: the same bundle under another
...
/** A shell, a resize, an interrupt and a ```ts block: what the tarball's Bun and trampolines exist for. */
async function drive(where: string, c: Awaited<ReturnType<typeof connect>>) {
  ...
  // The reason the tarball carries Bun at all: a ```ts fence runs on the server
  ...
  check(`${where}: a ts block ran on the Bun the release ships`, said.includes(`TS-42 ${BUN_VERSION}`) && ended?.exitCode === 0, JSON.stringify(said.trim().slice(-40)));
}

/** The release files the fixtures install from: dist-server's, plus a renamed copy for the update. */
function stageRelease(linux: NativeTarget, mac: NativeTarget | null) {
  mkdirSync(join(RELEASE, "next"), { recursive: true });
  const sums: Record<string, string> = {};
  for (const line of readFileSync(join(DIST, SUMS_FILE), "utf8").split("\n").filter(Boolean)) {
    const [sum, name] = line.split(/\s+/);
    for (const t of [linux, mac]) if (t && name === `${releaseName(version, t)}.tar.gz`) sums[`${t.platform}-${t.arch}`] = sum!;
  }
  const template = readFileSync(join(REPO, "release", "server.sh"), "utf8");
  for (const t of [linux, mac]) {
    if (!t) continue;
    const file = `${releaseName(version, t)}.tar.gz`;
    if (!existsSync(join(DIST, file))) throw new Error(`dist-server/ has no ${file}. Run \`bun run build:server -- --targets=${t.platform}-${t.arch}\`.`);
    copyFileSync(join(DIST, file), join(RELEASE, file));
  }
  // The script as a release renders it, with only the download address changed,
  // so `curl | sh` reads a file rather than GitHub and nothing else differs.
  writeFileSync(join(RELEASE, "server.sh"), renderInstallScript(template, { version, download: "file:///release", sums }));
  writeFileSync(join(RELEASE, "server-mac.sh"), renderInstallScript(template, { version, download: `file://${RELEASE}`, sums }));

  const repack = join(SCRATCH, "repack");
  mkdirSync(repack);
  run(["tar", "-xzf", join(RELEASE, `${releaseName(version, linux)}.tar.gz`), "-C", repack]);
  renameSync(join(repack, releaseName(version, linux)), join(repack, releaseName(NEXT, linux)));
  const nextFile = join(RELEASE, "next", `${releaseName(NEXT, linux)}.tar.gz`);
  run(["tar", "--no-xattrs", "-czf", nextFile, "-C", repack, releaseName(NEXT, linux)], { env: { COPYFILE_DISABLE: "1" } });
  const nextSum = new Bun.CryptoHasher("sha256").update(readFileSync(nextFile)).digest("hex");
  writeFileSync(
    join(RELEASE, "next", "server.sh"),
    renderInstallScript(template, { version: NEXT, download: "file:///release/next", sums: { [`${linux.platform}-${linux.arch}`]: nextSum } }),
  );
  return rm(repack, { recursive: true, force: true });
}
...
try {
  ...
  if (!existsSync(join(DIST, SUMS_FILE))) {
    console.error("no release in dist-server/. Run `bun run build:server` first.");
    process.exit(2);
  }
  await stageRelease(linux, mac);
