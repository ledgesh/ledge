#!/usr/bin/env bun
// Compile the PTY trampolines (src/bun/ptyNative.ts) into the dylib the app
// bundle ships. Runs as electrobun's `preBuild` hook, so every `bun run build`
// / `bun run dev` refreshes it before the copy map picks it up; also runnable
// on its own as `bun run build:native`.
//
// The point of doing this here rather than at runtime: TinyCC needs the SDK's
// system headers, and a Mac without Xcode or the Command Line Tools has none.
// This machine builds the app, so it has the SDK by construction — the user's
// does not have to. pty.ts keeps the in-process compile as its fallback.
...
const arches = target && ARCH_FLAGS[target] ? ARCH_FLAGS[target] : ["-arch", "arm64", "-arch", "x86_64"];
...
function compile(archFlags: string[]): { ok: boolean; stderr: string } {
  const p = Bun.spawnSync(
    ["cc", ...archFlags, `-mmacosx-version-min=${MIN_MACOS}`, "-O2", "-dynamiclib", "-o", OUT, SRC],
